...
${STYLE}`,
    ...
    piece(`stone_${color}`, `Treasure gem — ${color} (raised)`, 260 + i, {
...
${STYLE}`,
    ...
    piece(`stone_${color}_flat`, `Treasure tile — ${color} (flat)`, 270 + i, {
...
${STYLE}`,
  ...
  // ---- Cosmetics ----
  piece('bunny_ears', 'Bunny ears (frog cosmetic)', 280, {
    state: 'baked', width: 675, height: 612,
    instructions: `The bunny-skin cosmetic: a pair of upright rabbit ears drawn as one image, overlaid above the frog's head. Required size: exactly 675×612px, transparent PNG.
The canvas maps to the frog sprite: ears centered horizontally, canvas bottom near the frog's eye line (see reference, baked from the old drawn-in-code ears). Keep both ears in the same positions so they sit on the head correctly.
White outer ears with pink inners is the current look, but feel free to add floppiness or fur texture — it's a purchasable skin, so it should feel special.
${STYLE}`,
    reference: ref('bunny_ears.png', 675, 612),
  }),
