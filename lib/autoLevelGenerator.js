...
function solvePar(gridSize, frogCells, snakeCellsList, logCells, padCells, maxDepth, maxNodes, deadlineMs, snakeSaddled = []) {
  ...
  while (head < queue.length) {
    ...
    // Saddle (middle) cell of each saddled snake — a frog may land here after
    // a jump and then ride the snake when it slides.
    const saddleSet = new Set();
    for (let si = 0; si < snakes.length; si++) {
      if (snakeSaddled[si]) saddleSet.add(snakes[si][snakeMid[si]]);
    }
    ...
    for (let fi = 0; fi < frogs.length; fi++) {
      ...
      for (const [dc, dr] of dirs) {
        ...
        while (c >= 0 && c < gridSize && r >= 0 && r < gridSize) {
          ...
          if (isJumpover(here)) {
            // A free saddle is landable after a jump, and is still jump-over-able
            // (so the scan continues past it).
            if (jumped && saddleSet.has(here) && !frogSet.has(here)) {
