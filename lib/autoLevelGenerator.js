...
// Wizard: a frog hops a chain of logs into a portal, teleports across an
// impassable gap, then hops another chain to the pad. Par scales with the chain
// lengths (a on approach, b on exit), so a range of pars is produced for the
// daily difficulty bands to pick from. par = a + b.
export function generateWizardLevel(gridSize = 6, deadlineMs) {
  const n = Math.max(5, gridSize);
  const maxHop = Math.max(1, Math.floor((n - 1) / 2)); // hops that fit one row
  let attempts = 4000;
  while (attempts-- > 0) {
    if (deadlineMs !== undefined && Date.now() > deadlineMs) return null;
    const a = randInt(1, maxHop);
    const b = randInt(1, maxHop);
    const T = gridSymmetry(randInt(0, 7), n);
    const logs = [];
    // Approach chain along the top row: F, then `a` spaced logs; the a-th hop
    // lands on the portal mouth M1 (at col 2a).
    const F = T([0, 0]);
    for (let i = 0; i < a; i++) logs.push({ positions: [T([1 + 2 * i, 0])] });
    const M1 = T([2 * a, 0]);
    // Exit chain along the bottom row: M2, then `b` spaced logs to the pad.
    const M2 = T([0, n - 1]);
    for (let i = 0; i < b; i++) logs.push({ positions: [T([1 + 2 * i, n - 1])] });
    const P = T([2 * b, n - 1]);
    const level = {
      gridSize: n,
